"""CHANGE X matching / exchange-graph / chain-engine package."""

from .config import (
    CHANGE_CHAIN_ENABLED,
    CHANGE_CHAIN_MAX_LENGTH,
    CHAIN_ENGINE_VERSION,
    MATCHING_POLICY_VERSION,
    change_chain_enabled,
)
from .compatibility import CategoryCompatibility, get_compatibility
from .matchability import (
    chain_feature_enabled,
    is_chain_candidate,
    is_public_matchable,
    matchability_report,
)
from .preferences import get_user_preferences, public_preferences_view, set_user_preferences
from .scoring import MatchCandidate, NullScoreProvider, ScoreBreakdown, ScoreProvider
from .wants import WantValidationError, normalize_offer_fields, validate_structured_want
from .pair_score import DeterministicScoreProvider, can_form_edge
from .asset_lock import AssetLockError, NotImplementedAssetLock, get_asset_lock_provider

__all__ = [
    "CHANGE_CHAIN_ENABLED",
    "CHANGE_CHAIN_MAX_LENGTH",
    "CHAIN_ENGINE_VERSION",
    "MATCHING_POLICY_VERSION",
    "change_chain_enabled",
    "CategoryCompatibility",
    "get_compatibility",
    "chain_feature_enabled",
    "is_chain_candidate",
    "is_public_matchable",
    "matchability_report",
    "get_user_preferences",
    "public_preferences_view",
    "set_user_preferences",
    "MatchCandidate",
    "NullScoreProvider",
    "ScoreBreakdown",
    "ScoreProvider",
    "DeterministicScoreProvider",
    "can_form_edge",
    "WantValidationError",
    "normalize_offer_fields",
    "validate_structured_want",
    "AssetLockError",
    "NotImplementedAssetLock",
    "get_asset_lock_provider",
]
