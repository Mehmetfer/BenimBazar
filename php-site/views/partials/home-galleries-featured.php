...
/** @var list<array<string,mixed>> $featuredGalleries */
...
$hideGalleriesMore = !empty($hideGalleriesMore);
?>
<section class="home-galleries home-galleries--featured" id="one-cikan-galeriler" aria-label="Öne çıkan galeriler">
  <div class="home-galleries__head">
    <h2 class="home-galleries__title">Öne çıkan galeriler</h2>
    <?php if (!$hideGalleriesMore): ?>
    <a class="home-galleries__more" href="/galeriler">Tüm galeriler →</a>
    <?php endif; ?>
  </div>
  <div class="home-galleries__grid">
    <?php foreach ($featuredGalleries as $g):
        $gid = (int) ($g['id'] ?? 0);
    ...
    <a class="home-gallery-card<?= $isVip ? ' home-gallery-card--vip' : '' ?>" href="<?= cx_e($url) ?>">
      ...
      <div class="home-gallery-card__body">
        ...
        <?php endif; ?>
        <div class="home-gallery-card__meta"><?= $listingN ?> aktif araç<?php if ($new30 > 0): ?> · <?= $new30 ?> yeni / 30g<?php endif; ?></div>
      </div>
    </a>
    <?php endforeach; ?>
  </div>
